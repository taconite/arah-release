...
class H36MDataset(data.Dataset):
    ...
    def __init__(self, dataset_folder,
        ...
        with open(os.path.join(dataset_folder, subjects[0], 'Posing', 'cam_params.json'), 'r') as f:
            cameras = json.load(f)
        ...
        for subject in subjects:
            subject_dir = os.path.join(dataset_folder, subject, 'Posing')
    ...
    def get_mask(self, mask_in):
        ...
        if self.erode_mask and self.mode not in ['val', 'test']:
            border = 5
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.resize(image, (self.img_size[1], self.img_size[0]), interpolation=cv2.INTER_AREA)
        mask = cv2.resize(mask, (self.img_size[1], self.img_size[0]), interpolation=cv2.INTER_NEAREST)
        mask_erode = cv2.resize(mask_erode, (self.img_size[1], self.img_size[0]), interpolation=cv2.INTER_NEAREST)
