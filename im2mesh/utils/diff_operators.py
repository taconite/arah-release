...
def gradient(y, x, grad_outputs=None, retain_graph=True, create_graph=True):
...
def gradient_no_diff(y, x, grad_outputs=None):
    if grad_outputs is None:
        grad_outputs = torch.ones_like(y)
    grad = torch.autograd.grad(y, [x], grad_outputs=grad_outputs, retain_graph=True, create_graph=False)[0]
    return grad
