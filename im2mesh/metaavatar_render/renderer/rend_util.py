...
def get_body_bound_intersection(ray_directions, min_depth_val, max_depth_val):
    min_depth = (torch.reciprocal(ray_directions[..., -1]) * min_depth_val.view(-1, 1))
    max_depth = (torch.reciprocal(ray_directions[..., -1]) * max_depth_val.view(-1, 1))

    return torch.stack([min_depth, max_depth], dim=-1)
